...
marker_list <-
  read_tsv("data/Other/MetabolishMM_MarkerGenes_Selected.txt")

marker_list$Gene_name <-
  factor(marker_list$Gene_name, levels = unique(marker_list$Gene_name))
...
for (d in list$list) {
  ...
  (
    p1 <-
      ggplot(dat_lol , aes(Gene_name,  reorder(c, desc(
        c
      )))) + #reorder(variable, desc(variable)))
