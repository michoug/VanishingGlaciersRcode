...
dat_cov_virus <- read_tsv("data/vMAGs_cov_norm.txt.gz")
  ...
  mutate(
    goodTax = case_when(
      Class == "Caudoviricetes" &
        is.na(Order) ~ "Caudoviricetes_Unclassified",
      Class == "Caudoviricetes" &
        Order == "unclassifiedCaudoviricetes" ~ "Caudoviricetes_Unclassified",
...
colors <-
  colorRampPalette(brewer.pal(9, "Set1"))(length(unique(datToPlot$Tax)))
