...
dat_Tara <-
  read_tsv("../Prokaryotes/MAGs_Comparison/Tara_NOMIS.txt", col_names = F)
dat_Busi <-
  read_tsv("../Prokaryotes/MAGs_Comparison/Busi_NOMIS.txt", col_names = F)
dat_Ensemble <-
  read_tsv("../Prokaryotes/MAGs_Comparison/NOMIS_ENSEMBLE.txt",
           col_names = F)
dat_TG2G <-
  read_tsv("../Prokaryotes/MAGs_Comparison/TG2G_NOMIS.txt", col_names = F)
...
filter_table <- function(dat) {
  colnames(dat) <-
    c("NOMIS", "Data", "ANI", "NOMIS_cont", "Data_cont")
...
df_temp <-
  map_df(list(dat_Tara, dat_Busi, dat_Ensemble, dat_TG2G), filter_table)
