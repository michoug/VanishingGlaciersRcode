...
dat_prok <- read_tsv("../VirusGenomad_Virus_host/NOMIS_genomad_virus_summary.txt")
dat_euk_vtax <- read_tsv("../VirusGenomad_Virus_Eukaryotes/allEukviruses.names.out", col_names = F)
dat_prok_vtax <- read_tsv("../VirusGenomad_Virus_host/allhostviruses.names.out", col_names = F)
...
dat_virus_euk <- dat_euk%>%
  left_join(dat_euk_vtax,by = c("seq_name"),relationship = "many-to-many")%>%
  mutate(VTax = if_else(
...
dat_virus_euk$goodVirusTax <- gsub(" ", "", dat_virus_euk$goodVirusTax)

dat_virus_sank <- dat_virus_euk %>%
  make_long(goodVirusTax, taxa_good)
...
p1
# ggsave_fitmax("Sankey_Genomad_Euk.pdf", p1)
...
p2 <- ggplot(dat_virus_sank_p, aes(x = x, 
                                 next_x = next_x, 
                                 node = node, 
                                 next_node = next_node,
                                 fill = factor(node), label = node)) +
...
p2
ggsave_fitmax("Sankey_Genomad_Prok.pdf", p1)
