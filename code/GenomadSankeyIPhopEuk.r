...
dat_virus_euk <- dat_euk%>%
  ...
  mutate(goodVirusTax = case_when(
    V_Class == "Caudoviricetes" & is.na(V_Order) ~ "Caudoviricetes Unclassified",
    V_Class == "Caudoviricetes" & V_Order == "unclassifiedCaudoviricetes" ~ "Caudoviricetes Unclassified",
...
colnames(dat_virus_euk) <- c("seq_name","Viral Taxonomy","MAGs","Eukaryotic Taxonomy")
...
p1
ggsave_fitmax("Figures/Sankey_Genomad_Euk.pdf", p1)
