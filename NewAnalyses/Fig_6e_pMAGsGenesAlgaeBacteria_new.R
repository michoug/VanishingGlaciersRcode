...
dat_quorum <- read_tsv("NewAnalyses/pMAGs_QuorumSensing_sub.txt")
...
dat_quorum_genes <- dat_quorum %>%
  select(HmmscanResults)%>%
  filter(HmmscanResults != "HmmscanResults")%>%
  mutate(Category = "quorum_sensing")%>%
  distinct()%>%
  rename(KO = HmmscanResults)

all_genes_quorum <- rbind(all_genes, dat_quorum_genes)


all_genes_final <- all_genes_quorum %>%
  distinct() %>%
  group_by(Category) %>%
  mutate(number = n())
...
dat_filter <- dat_select %>%
  ...
  reframe(percOfGenes = n() / number) %>%
  ungroup() %>%
  distinct() %>%
  complete(MAGs, Category, fill = list(percOfGenes = 0)) %>%
  right_join(cluster, by = c("MAGs" = "sub_grp_6"))

dat_quorum_clean <- dat_quorum %>%
  mutate(check_value = if_else(HmmscanResults == DiamondResults, T, F))%>%
  filter(check_value == T)%>%
  select(-check_value)%>%
  select(GeneName, HmmscanResults)%>%
  mutate(MAGs = gsub("(.*)_\\d+_\\d+", "\\1", GeneName))%>%
  filter(!(MAGs %in% magsRem$MAGs))%>%
  right_join(
    all_genes_final,
    by = c("HmmscanResults" = "KO"),
    multiple = "all",
    relationship = "many-to-many"
  )%>%
  group_by(MAGs, HmmscanResults, Category, number) %>%
  summarise(n = n()) %>%
  mutate(n_presence = if_else(n > 0, 1, 0))%>%
  ungroup() %>%
  group_by(MAGs, Category, number) %>%
  reframe(percOfGenes = n() / number) %>%
  ungroup() %>%
...
dat_concatenate <- rbind(dat_filter, dat_quorum_clean)

dat_plot <- dat_concatenate %>%
  group_by(d, Category) %>%
  summarise(percOfPresence = mean(percOfGenes)) %>%
  ungroup() %>%
  complete(d, Category, fill = list(percOfPresence = 0)) %>%
  mutate(
    d = case_when(
      d == "1" ~ "4 - Diverse Taxa",
      d == "2" ~ "5 - Myxcoccota &\nBdellovibrionata",
...
dat_plot_t <- dat_plot %>%
  pivot_wider(names_from = Category, values_from = percOfPresence)%>%
  select(-"NA")
...
p1 <-
  ggplot(dat_plot, aes(x = as.character(d), y = Category, fill = percOfPresence*100)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "darkgreen") +
  labs(x = NULL, y = NULL, fill = "Proprotion (%)") +
  theme_pubr()
p1
ggsave_fitmax("Figures/Fig_6e_Heatmap_cat_algae_bact_new.pdf", p1, maxwidth = 12)
